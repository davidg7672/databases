...
FROM country c
    JOIN province p USING (country_code) -- city in the same country
    JOIN city ci USING (province_name) -- making sure city is in the same province
...
FROM province p
    ...
    JOIN city ci1 USING (province_name)
    JOIN city ci2 USING (province_name)
WHERE ci1 <> ci2 AND
    ci1.population > 500000 AND
    ci2.population > 500000;
...
SELECT ci1.city_name, ci1.province_name, ci1.country_code, ci1.population,
       ci2.city_name, ci2.province_name, ci2.country_code, ci2.population
...
WHERE p.area <= 50000  -- small province area
    AND c.gdp >= 15000  -- high GDP country
